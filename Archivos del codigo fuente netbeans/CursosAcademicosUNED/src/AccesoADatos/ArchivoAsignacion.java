...
///
///maneja la asignación de cursos a estudiantes en archivos .txt
///cada archivo se nombra con el ID del estudiante y contiene hasta 4 códigos de cursos
///
public class ArchivoAsignacion {

    private static final String CARPETA_ASIGNACIONES = "asignaciones_estudiantes";

    ///
    ///agrega un curso al archivo del estudiante si no ha alcanzado el máximo de 4
    ///retorna un mensaje con el resultado
    ///
    public static String asignarCurso(String idEstudiante, String codigoCurso) {
        try {
            //crear carpeta si no existe
            File carpeta = new File(CARPETA_ASIGNACIONES);
            if (!carpeta.exists()) {
                carpeta.mkdir();
            }

            //archivo del estudiante
            File archivo = new File(carpeta, idEstudiante + ".txt");

            //leer cursos ya asignados
            List<String> cursosAsignados = new ArrayList<>();
            if (archivo.exists()) {
                BufferedReader reader = new BufferedReader(new FileReader(archivo));
                String linea;
                while ((linea = reader.readLine()) != null) {
                    cursosAsignados.add(linea);
                }
                reader.close();
            }

            //validar si ya tiene 4 cursos
            if (cursosAsignados.size() >= 4) {
                return "Este estudiante ya tiene 4 cursos asignados.";
            }

            //validar si ya está asignado ese curso
            if (cursosAsignados.contains(codigoCurso)) {
                return "Este curso ya está asignado a este estudiante.";
            }

            //agregar nuevo curso
            BufferedWriter writer = new BufferedWriter(new FileWriter(archivo, true));
            writer.write(codigoCurso);
            writer.newLine();
            writer.close();
            ...
            return "Error al asignar el curso: " + e.getMessage();
        }
    }

    ///
    ///obtiene la lista de cursos asignados a un estudiante
    ///retorna lista vacía si no hay asignaciones
    ///
    public static List<String> obtenerCursosAsignados(String idEstudiante) {
        List<String> cursos = new ArrayList<>();
        File archivo = new File(CARPETA_ASIGNACIONES, idEstudiante + ".txt");
