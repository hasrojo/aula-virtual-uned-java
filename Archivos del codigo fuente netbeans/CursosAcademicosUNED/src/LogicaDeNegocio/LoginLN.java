...
///
///logica de negocio para validar el inicio de sesión.
/// Permite el acceso como administrador (admin/admin)
/// o como estudiante con doble factor (respuesta secreta)
///
public class LoginLN {

    ///
    ///valida usuario, contraseña y respuesta secreta
    /// Retorna:
    /// - "admin" si es el usuario fijo admin/admin
    /// - "estudiante" si el ID, contraseña y respuesta coinciden con un archivo válido
    /// - "error" en cualquier otro caso
    ///
    public static String validarCredenciales(String usuario, String contrasena, String respuestaSecreta) {
        //validación fija de administrador
        if (usuario.equals("admin") && contrasena.equals("admin")) {
            return "admin";
        }

        try {
            //buscar archivo del estudiante por ID
            Estudiante estudiante = ArchivoEstudiante.cargarEstudiantePorId(usuario);

            //validar credenciales y respuesta secreta
            if (estudiante != null &&
                estudiante.getContrasena().equals(contrasena) &&
                estudiante.getRespuestaSecreta().equalsIgnoreCase(respuestaSecreta)) {
    ...
    private static Estudiante estudianteAutenticado;



public static Estudiante getEstudianteAutenticado() {
